...
export default function Dashboard() {
  ...
  const fetchRecentOrders = async () => {
    try {
      // Replace with actual order fetching when order utils are available
      ...
      // Dummy data for now
      setRecentOrders([
        { id: 1, customer_name: "John Doe", total: 150000, created_at: new Date().toISOString() },
        { id: 2, customer_name: "Jane Smith", total: 200000, created_at: new Date(Date.now() - 86400000).toISOString() },
        { id: 3, customer_name: "Bob Wilson", total: 175000, created_at: new Date(Date.now() - 172800000).toISOString() }
      ]);
    } catch (error) {
    ...
    <main className="container mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-tertiary p-6 rounded-xl shadow-lg">
          <h2 className="text-lg font-medium mb-4">Aktivitas 7 Hari Terakhir</h2>
          <div className="h-64">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {/* Kegiatan Terbaru */}
        <div className="bg-tertiary p-6 rounded-xl shadow-lg">
          <h2 className="text-lg font-medium mb-4">Kegiatan Terbaru</h2>
          <div className="space-y-4">
            {recentActivities.length > 0 ? (
              recentActivities.map((activity: any, index: number) => (
                <div key={activity.id || index} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  {/* Activity Image */}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm truncate">
                      {activity.title || activity.name || `Kegiatan ${index + 1}`}
                    </p>
              ...
              <div className="py-8 text-gray-400 text-center">
                <CalendarCheck size={48} className="mx-auto mb-2 text-gray-300" />
        ...
        {/* Produk Terbaru */}
        <div className="bg-tertiary p-6 rounded-xl shadow-lg">
          <h2 className="text-lg font-medium mb-4">Produk Terbaru</h2>
          <div className="space-y-4">
            {recentProducts.length > 0 ? (
              recentProducts.map((product: any, index: number) => (
                <div key={product.id || index} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  {/* Product Image */}
                  <div className="w-12 h-12 rounded-lg overflow-hidden bg-gray-200 flex-shrink-0">
                    {product.image ? (
                      <img 
        ...
        {/* Order Terbaru */}
        <div className="bg-tertiary p-6 rounded-xl shadow-lg">
          <h2 className="text-lg font-medium mb-4">Order Terbaru</h2>
          <div className="space-y-4">
            {recentOrders.length > 0 ? (
              recentOrders.map((order: any, index: number) => (
                <div key={order.id || index} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  {/* Customer Avatar */}
